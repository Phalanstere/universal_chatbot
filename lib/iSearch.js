...
var ISearch = function( speechact ) {
    ...
    digz("Anzahl potenzielle Items " + self.valid_entries.length);
    }
    ...
    this.excluded_digest = function(element, results, offset ) {
        ...
        if ( inf.valid ) {
            // digz ( inf.list );
            digz ( self.params.type +  " --- Aufgabe bewältigt: " + inf.valid + " für das Element " + element.type  + " -- INKREMENT: " + inf.inc );
    ...
    this.process_filtered_sequence = function( sequence, offset ) {  
        ...
        // for (var i = 0; i < 4; i++) {
            ...
            if (valid) {
                ...
                if ( i === 4) digz("Ende der Funktion");
        
            }
        }
        
        digz("Funktion beendet - sollte das Ergebnis in eine Datei schreiben");
        digz("Anzahl valid_entries " + self.valid_entries.length);
        // digx( filtered );

        self.store_filtered_results();
    ...
    this.process = function( params, offset ) {
        ...
        else self.process_filtered_sequence(  params.sequence, offset  );
